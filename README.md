...
当前版本 v0.9.0。换一家商家使用，**不需要改任何代码**——运行 `npm run init` 生成配置后，直接在「基础数据」页补齐客户与商品即可营业。

## 功能

以下能力均可在代码中对应验证（`server.js`、`public/`）：

- **六个页面模块**，hash 路由切换：经营总览 `#overview`、订单中心 `#orders`、库存与拣货 `#warehouse`、配送调度 `#delivery`、基础数据 `#master`、经营分析 `#analysis`、系统与账本 `#settings`。
...
覆盖：健康检查与配置接口、订单状态机各闸门（含 force 旁路已移除、resolve 状态守卫、编辑仅限确认前）、配送状态只进不退、已送达不可取消、拣货扣库存、缺货拒绝、库存调整、实时经营分析、CSV/xlsx 导入与导出回环、主数据（商品/客户/车辆）增删改与级联删除、数据文件缺失自动播种、损坏返回 500 不崩溃、审计截断 500 条、操作员入审计、每日备份滚动保留、跨站 Origin 拒绝、非 JSON 内容类型 415、订单 ID 唯一、访问口令登录/登出失效。另有 GitHub Actions 在 Node 18/20/22 上自动运行同一套测试。
