...
- **当前版本 v0.8.0**：零第三方运行依赖，Node.js ≥ 18。
...
| 能力 | 说明 |
| --- | --- |
| **六大页面模块** | hash 路由切换：经营总览 `#overview`、订单中心 `#orders`、库存与拣货 `#warehouse`、配送调度 `#delivery`、经营分析 `#analysis`、系统与账本 `#settings` |
...
- **鉴权现状**：当前无登录鉴权，默认只监听本机；开放局域网前请确认网络环境可信（角色权限在后续计划中）。
- **备份建议**：生产使用前请定期通过"系统与账本"页的备份链接（`/api/export`）或手工复制数据文件备份。本项目**不是速达官方插件**，不会绕过速达权限或直接操作其生产数据库。
